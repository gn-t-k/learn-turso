...
import { setTaskIncomplete } from "./set-task-incomplete.server";

describe("setTaskIncomplete", () => {
	beforeEach(async () => {
		await resetTables();
	});
	...
	describe("存在するタスクIDを指定した場合", () => {
		...
		beforeEach(async () => {
			await withTestDb(async (database) => {
				createdTask = await factories
					.tasks(database)
					.create({ completed: true });

				await setTaskIncomplete(createdTask.id);
		...
		test("タスクのcompletedがfalseに更新される", () => {
	...
	describe("存在しないタスクIDを指定した場合", () => {
		...
		test("TaskNotFoundErrorが投げられる", async () => {
			await withTestDb(async () => {
				await expect(setTaskIncomplete(nonExistentTaskId)).rejects.toThrowError(
					"存在しないタスクのIDが指定されました",
				);
			});
		});
		...
		test("どのタスクも更新されない", async () => {
			await withTestDb(async (database) => {
				...
				try {
					await setTaskIncomplete(nonExistentTaskId);
				} catch {
					// エラーは無視
				}
